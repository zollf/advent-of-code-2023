...
defmodule AdventOfCode.Day17.P2 do
  @max_con 10
  @min_con 4
  ...
  def traverse(map, target, queue, visited) do
    ...
    if steps >= 4 && cur_coords == target do
      heat
    else
      neighbors =
        map
        ...
        |> Enum.reject(fn {_, neighbor_direction} ->
          neighbor_direction != direction && steps > 0 && steps < @min_con
        end)
...
AdventOfCode.Day17.P2.traverse(map, {x_max - 1, y_max - 1}, queue, MapSet.new())
|> IO.inspect()
